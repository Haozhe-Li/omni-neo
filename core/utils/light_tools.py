...
def arxiv_search_light(
    ...
    k (int): The number of results to return. Default is 5. Max is 5.
    """
    _ = state
    return arxiv_search(query=query, k=k)


@tool
def python_code_sandbox_light(
    code: str,
    state: Annotated[dict[str, Any], InjectedState] = None,
) -> str:
    """
    ...
    """
    _ = state
    return run_python_tool(code=code)
