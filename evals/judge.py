...
class GroundingScore(BaseModel):
    supported: bool = Field(description="True if the source text supports the claim.")
...
_GROUNDING_SYSTEM = """You verify citations. Given a CLAIM from an AI assistant's answer and the
SOURCE TEXT(s) it cited, decide whether the sources — taken together — actually
support the claim.

Supported means the sources state the claim, or state something the claim
follows from directly. A source merely being on the same topic is NOT support.
When a claim cites multiple sources, they combine: content covered by any one
of them counts, and you are checking whether their union backs the claim, not
whether each source individually covers all of it.

Be strict: an unsupported citation is worse than no citation, because it looks
verified."""


def _extract_citation_claims(text: str) -> list[tuple[str, list[int]]]:
    """One claim per line, each paired with every citation number it carries.

    Deliberately per-*line*, not per blank-line-delimited paragraph
    (`parsers.paragraphs`). The product's own citation convention batches a
    paragraph's citations at its end, and consecutive Markdown bullets carry no
    blank line between them — so paragraph-splitting merges an entire bulleted
    list into one block and collapses everything down to whichever citation
    number happens to be lowest. A report with

        - 体型与体重...[1]
        - 外耳...[1][12]
        - 鳍肢...[1]

    would merge into a single three-fact "claim" graded only against source
    [1], guaranteeing the [12]-specific fact (and generally most multi-bullet
    blocks) reads as unsupported — not because the citation was wrong, but
    because the grading unit was too coarse. This produced a 0%-support result
    across every run before the fix, on cases whose citations were otherwise
    fine.

    Splitting on lines instead makes each bullet — the actual unit the model
    attached one citation cluster to — its own claim, and every number in that
    cluster travels with it rather than only the smallest one.

    Reports here are model-generated Markdown without manual mid-paragraph line
    wrapping (confirmed empirically), so a genuine multi-sentence paragraph is
    still one line and is not fragmented by this.
    """
    from evals import parsers
    ...
    for line in text.splitlines():
        ...
        if not line:
            continue
        cited = sorted(parsers.find_citations(line).distinct)
        if cited:
            out.append((line[:1200], cited))
    return out


async def judge_citation_grounding(
    trace: RunTrace,
    *,
    model: str = DEFAULT_JUDGE_MODEL,
    max_claims: int = 10,
) -> JudgeResult | None:
    """Layer-4 citation checking: does the cited source support the sentence?

    Uses the full tool output backing each `[n]` (see
    `RunTrace.source_text_for`), not the registry's truncated snippet — judging
    against a snippet would report correct citations as unsupported whenever the
    supporting sentence fell past the cutoff.

    Sampled to `max_claims` because this is one LLM call per claim and a
    deep-research answer can carry sixty.
    """
    from evals import parsers
    ...
    for turn in trace.turns:
        text = turn.text
        for report in parsers.extract_reports(text):
            text += "\n\n" + report.body
        claims.extend(_extract_citation_claims(parsers.prose_only(text)))
    if not claims:
        return None

    step = max(1, len(claims) // max_claims)
    sampled = claims[::step][:max_claims]

    # Per-source budget shrinks as a claim cites more sources, so a claim with
    # three citations doesn't send 18000 chars to the judge — the total stays
    # bounded regardless of how many sources back one line.
    llm = _get_judge_llm(model).with_structured_output(GroundingScore)
    async def _one(claim: str, numbers: list[int]):
        per_source_cap = max(1500, 6000 // len(numbers))
        blocks = []
        for n in numbers:
            source = trace.source_text_for(n)
            if source:
                blocks.append(f"[{n}]:\n{source[:per_source_cap]}")
        if not blocks:
            return None
        try:
    ...
    verdicts = [v for v in await asyncio.gather(*[_one(c, ns) for c, ns in sampled]) if v]
    if not verdicts:
        return None
    supported = sum(1 for v in verdicts if v.supported)
    ratio = supported / len(verdicts)
    return JudgeResult(
        key="judge:citation_grounding",
